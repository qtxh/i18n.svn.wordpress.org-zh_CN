...
if ( version_compare( '4.3', phpversion(), '>' ) ) {
	die( 'Your server is running PHP version ' . phpversion() . ' but WordPress requires at least 4.3. WordPress至少要求PHP 4.3版本' );
}

if ( !extension_loaded('mysql') && !file_exists(ABSPATH . 'wp-content/db.php') )
	die( 'Your PHP installation appears to be missing the MySQL extension which is required by WordPress. 服务器PHP安装里没有 MySQL 数据库扩展.' );
...
if ( is_wp_error($prefix) )
	wp_die('<strong>ERROR</strong>: <code>$table_prefix</code> in <code>wp-config.php</code> can only contain numbers, letters, and underscores. 数据库前缀只能使用字母数据和下划线');
