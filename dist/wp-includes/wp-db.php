...
class wpdb {
	...
	function __construct($dbuser, $dbpassword, $dbname, $dbhost) {
		...
		if (!$this->dbh) {
			$this->bail("
<h1>有错误发生！不能建立数据库连接！</h1>
<p>问题有可能是配置文件 <code>wp-config.php</code> 里面的用户名和密码有误 或者是 不能连接到数据库主机 <code>$dbhost</code>. 也可能是你的主机提供商的数据库服务器出现了问题.</p>
<ul>
	<li>请检查数据库用户名和密码！</li>
	<li>请检查数据库主机名！</li>
	<li>请检查数据库服务器是否在运行！</li>
</ul>
<p>如果你知道问题所在，请回到配置页面或手动修改 <code>wp-config.php</code> 文件. 如果你对于这些设置不太清楚，请联系你的主机提供商. 如果问题还是不能解决，你也可以试试 <a href='http://wordpress.org/support/'>WordPress 技术支持论坛</a>.</p>
");
			return;
		}
